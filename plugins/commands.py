...
from info import ADMINS, THREE_VERIFY_GAP, LOG_CHANNEL, USERNAME, VERIFY_IMG, IS_VERIFY, AUTH_CHANNEL, SHORTENER_WEBSITE, SHORTENER_API, SHORTENER_WEBSITE2, SHORTENER_API2, SHORTENER_API3, SHORTENER_WEBSITE3, LOG_API_CHANNEL, TWO_VERIFY_GAP, TUTORIAL, TUTORIAL2, TUTORIAL3, QR_CODE, DELETE_TIME, OWNER_LINK, MOVI_GRP, MOVI_CHNL, PICZ
...
async def start(client: Client, message):
    ...
    if len(message.command) != 2:
        ...
        await message.reply_photo(
            photo=random.choice(PICZ),
            caption=script.START_TXT.format(message.from_user.mention, get_status()),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        return
    ...
    id = rfsub_id if rfsub_id else int(AUTH_CHANNEL)
    channel = int(id)
...
async def tutorial(bot, message):
    ...
    if user.status != enums.ChatMemberStatus.ADMINISTRATOR and user.status != enums.ChatMemberStatus.OWNER and str(userid) not in ADMINS:
        ...
        return
    try:
        tutorial = re.findall(r"(?P<url>https?://[^\s]+)", message.text)[0]
    except:
...
async def tutorial_two(bot, message):
    ...
    if user.status != enums.ChatMemberStatus.ADMINISTRATOR and user.status != enums.ChatMemberStatus.OWNER and str(userid) not in ADMINS:
        ...
        return
    try:
        tutorial_two = re.findall(r"(?P<url>https?://[^\s]+)", message.text)[0]
    except:
...
async def tutorial_three(bot, message):
    ...
    if user.status != enums.ChatMemberStatus.ADMINISTRATOR and user.status != enums.ChatMemberStatus.OWNER and str(userid) not in ADMINS:
        ...
        return
    try:
        tutorial_three = re.findall(r"(?P<url>https?://[^\s]+)", message.text)[0]
    except:
...
async def set_rfsub(client, message):
    try:
        ...
        return await message.reply_text('<b>Make sure the ID is an integer.</b>')
    
    try:
        chat = await client.get_chat(channel_id)
        if chat.type != enums.ChatType.CHANNEL:
            return await message.reply_text(f"<b><code>{channel_id}</code> is not a channel, send only channel ID.</b>")
        await db.set_rfsub_id(channel_id)
        await db.del_join_req()  # Clear existing join requests
        mention = message.from_user.mention
        await client.send_message(LOG_CHANNEL, f"#Rfsub_Channel_set\n\nUser - {mention} set the request force-subscribe channel:\n\nChannel - {chat.title}\nID - `{channel_id}`")
        await message.reply_text(f"<b>✅ Successfully set request force-subscribe channel\n\nChannel Name - {chat.title}\nID <code>{channel_id}</code></b>")
    except Exception as e:
        await message.reply_text(f"<b>Error setting channel: <code>{e}</code>\nMake sure the bot is an admin in that channel.</b>")

@Client.on_message(filters.command('norfsub') & filters.private & filters.user(ADMINS))
async def remove_rfsub(client, message):
    rfsub_id = await db.get_rfsub_id()
    if not rfsub_id:
        return await message.reply_text("<b>No request force-subscribe channel set yet 🤪</b>")
    await db.remove_rfsub_id()
    await db.del_join_req()  # Clear existing join requests
    mention = message.from_user.mention
    await client.send_message(LOG_CHANNEL, f"#Remove_Rfsub_Channel\n\nUser - {mention} removed the request force-subscribe channel.")
    await message.reply_text("<b>✅ Successfully removed request force-subscribe channel. Reverted to default AUTH_CHANNEL.</b>")
