...
from info import MAX_BTN, BIN_CHANNEL, USERNAME, URL, IS_VERIFY, LANGUAGES, AUTH_CHANNEL, SUPPORT_GROUP, QR_CODE, DELETE_TIME, PM_SEARCH, ADMINS, MOVI_GRP
...
async def pm_search(client, message):
    if PM_SEARCH:
        ...
        return await message.reply_text(
            f"<b>ʜᴇʏ {mentn} 😇</b>\n\n"
            f"<blockquote><b><i>🚫 ᴡᴇ ᴀʀᴇ ғᴏʀᴄᴇᴅ ᴛᴏ ᴅɪsᴀʙʟᴇ ᴘᴍ sᴇᴀʀᴄʜ ᴅᴜᴇ ᴛᴏ ʜɪɢʜ sᴇʀᴠᴇʀ ʟᴏᴀᴅ.\n\n"
            f"🎬 ғᴏʀ ɴᴏᴡ, ʏᴏᴜ ᴄᴀɴ ʀᴇǫᴜᴇsᴛ ᴍᴏᴠɪᴇs ᴏʀ sᴇʀɪᴇs ғʀᴏᴍ ᴏᴜʀ ɢʀᴏᴜᴘs ʙᴇʟᴏᴡ.\n\n"
            f"⚡ ᴊᴏɪɴ ᴏᴜʀ ᴄʜᴀɴɴᴇʟs ᴀɴᴅ ɢʀᴏᴜᴘs ғᴏʀ ɪɴsᴛᴀɴᴛ ᴍᴏᴠɪᴇ ᴜᴘᴅᴀᴛᴇs!</blockquote></i></b>\n\n"
            f"<b>💡 ᴛɪᴘ: ᴊᴜsᴏ ᴛʏᴘᴇ ʏᴏᴜʀ ᴍᴏᴠɪᴇ ɴᴀᴍᴇ ɪɴ ᴛʜᴇ ɢʀᴏᴜᴘ ᴀɴᴅ ɢᴇᴛ ɪɴsᴛᴀɴᴛ ʀᴇsᴜʟᴛs! 🚀</b>",
...
async def group_search(client, message):
    ...
    if settings["auto_filter"]:  
        ...
        link_pattern = r'(https?://[^\s]+|www\.[^\s]+|t\.me/[^\s]+|@[^\s]+|bit\.ly/[^\s]+|tinyurl\.com/[^\s]+|[^\s]+\.[a-zA-Z]{2,}/[^\s]*|[^\s]+\.[a-zA-Z]{2,})'
        if re.search(link_pattern, message.text, re.IGNORECASE):
            ...
            try:
                ...
                alert_msg = await message.reply_text(
                    f"<b>{user_mention}'s ᴍᴇssᴀɢᴇ ᴡᴀs ɪᴅᴇɴᴛɪꜰɪᴇᴅ ᴀs sᴘᴀᴍ ᴀɴᴅ ᴛʜᴇʏ'ᴠᴇ ʙᴇᴇɴ ᴍᴜᴛᴇᴅ ꜰᴏʀ 24ʜ ✅\n\n"
                    f"ᴘʟᴇᴀsᴇ ᴅᴏɴ'ᴛ sᴇɴᴅ ʟɪɴᴋs ᴛᴏ ᴛʜɪs ɢʀᴏᴜᴘ ‼️</b>"
                )
...
async def lang_search(client: Client, query: CallbackQuery):
    ...
    if settings["link"]:
        ...
        btn = [[
            InlineKeyboardButton(text=f"{get_size(file.file_size)} ➛ {get_name(file.file_name)}", url=f'https://t.me/{temp.U_NAME}?start=file_{reqnxt}_{file.file_id}')]
               for file in files
              ]
    if n_offset != "":
        btn.append(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"<b><i><blockquote>Hᴇʏ Rᴇꜱᴜʟᴛꜱ Fᴏʀ Yᴏᴜʀ Sᴇᴀʀᴄʜ</blockquote></i></b>"
