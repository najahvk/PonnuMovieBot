...
class Database:
    ...
    async def set_rfsub_id(self, rfsub_id: int):
        """Set the global rfsub_id."""
        await self.misc.update_one(
            {"key": "rfsub_id"},
            {"$set": {"rfsub_id": rfsub_id}},
            upsert=True
        )

    async def get_rfsub_id(self):
        """Retrieve the global rfsub_id."""
        result = await self.misc.find_one({"key": "rfsub_id"})
        return result.get("rfsub_id") if result else None

    async def remove_rfsub_id(self):
        """Remove the global rfsub_id."""
        await self.misc.delete_one({"key": "rfsub_id"})
